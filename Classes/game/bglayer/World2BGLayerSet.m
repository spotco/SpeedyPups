...
-(World2BGLayerSet*)cons {
	...
	backislands = [BackgroundObject backgroundFromTex:[Resource get_tex:TEX_BG2_FRONTISLANDS_1] scrollspd_x:0.05 scrollspd_y:0.006];
	frontislands = [BackgroundObject backgroundFromTex:[Resource get_tex:TEX_BG2_FRONTISLANDS_0] scrollspd_x:0.07 scrollspd_y:0.007];
...
-(void)set_scrollup_pct:(float)pct{
	...
	[backislands setPosition:ccp(backislands.position.x,backislands.position.y-500*pct)];
	[frontislands setPosition:ccp(frontislands.position.x,frontislands.position.y-500*pct)];
}
